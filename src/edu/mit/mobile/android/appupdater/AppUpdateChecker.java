...
/**
 * A fairly simple non-Market app update checker. Give it a URL pointing to a
 * JSON file and it will compare its version (from the manifest file) to the
 * versions listed in the JSON. If there are newer version(s), it will provide
 * the changelog between the installed version and the latest version. The
 * updater checks against the versionCode, but displays the versionName. While
 * you can create your own OnAppUpdateListener to listen for new updates,
 * OnUpdateDialog is a handy implementation that displays a Dialog with a
 * bulleted list and a button to do the upgrade. The JSON format looks like
 * this:
 * 
 * <pre>
 *   {
...
public class AppUpdateChecker {
     ...
     */
    public AppUpdateChecker(Context context, String versionListUrl,
            OnAppUpdateListener updateListener) {
        mContext = context;
        mVersionListUrl = versionListUrl;
        mUpdateListener = updateListener;

        try {
            currentAppVersion = context.getPackageManager().getPackageInfo(
        ...
        PreferenceManager.setDefaultValues(mContext, SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE,
                R.xml.preferences, false);
    }

    // min interval is stored as a string so a preference editor could
    // potentially edit it using a text edit widget
    ...
    /**
     * You normally shouldn't need to call this, as {@link #checkForUpdates()}
     * checks it before doing any updates.
     * 
     * @return true if the updater should check for updates
     */
    public boolean isStale() {
        return System.currentTimeMillis() - mPrefs.getLong(PREF_LAST_UPDATED, 0) > getMinInterval()
                * MILLISECONDS_IN_MINUTE;
    }

    /**
     * Checks for updates if updates haven't been checked for recently and if
     * checking is enabled.
     */
    ...
    /**
     * Checks for updates regardless of when the last check happened or if
     * checking for updates is enabled.
     */
    ...
    private void triggerFromJson(JSONObject jo) throws JSONException {
        ...
        final String latestVersionName = versionMap.get(latestVersionNumber).getString(
                "versionName");
        ...
        for (final Entry<Integer, JSONObject> version : versionMap.headMap(currentAppVersion)
                .entrySet()) {
        ...
        protected JSONObject doInBackground(String... params) {
            ...
            try {
                ...
                if (statusCode != HttpStatus.SC_OK) {
                    ...
                    if (e.getContentType().getValue().equals("text/html")
                            || e.getContentLength() > 40) {
                        // long response body. Serving HTML...
                        throw new VersionCheckException("Got a HTML instead of expected JSON.");
                    }
                    throw new VersionCheckException("HTTP " + res.getStatusLine().getStatusCode()
