 ...
 * be loaded using a wrapped {@link ImageLoaderAdapter}. Multiple image fields
 * can be mapped to the same image view and the first one with a valid image
 * will be loaded. Additionally, it shows an indeterminate progress bar when the
 * adapter is null. This progress bar will go away once the adapter has been
 * set.
 * 
 * @author steve
 */
public class SimpleThumbnailCursorAdapter extends SimpleCursorAdapter {
     ...
     * @param layout
     * @param c
     * @param from You can load alternate images by specifying multiple from IDs
     *            mapping to the same TO ID. This only works for images, as
     *            listed below.
     * @param to
     * @param imageIDs a list of ImageView IDs whose images will be loaded by
     *            this adapter.
     * @param flags
     */
