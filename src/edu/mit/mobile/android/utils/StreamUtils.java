...
public class StreamUtils {
    ...
    /**
     * Reads a single line from the given input stream, blocking until it gets a
     * newline.
     * 
     * @param in InputStream to read from.
     * @return A single line, without the newline at the end or null if no data
     *         could be read.
     * @throws IOException
     */
    static public String readLine(InputStream in) throws IOException {
        ...
        for (int b = in_reader.read(); b != -1; b = in_reader.read()) {
            receivedData = true;
            if (b == '\r') {
                continue;
            }
    ...
    static public String inputStreamToString(InputStream in) throws IOException {
        ...
        for (int readBytes = in_reader.read(cbuf, 0, bufsize); readBytes > 0; readBytes = in_reader
                .read(cbuf, 0, bufsize)) {
