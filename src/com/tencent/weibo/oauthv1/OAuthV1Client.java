 ...
 * 
 * <pre>
 * OAuthV1Client.setQHttpClient(QHttpClient qHttpClient)
 * </pre>
 * 
 * <br>
 * 为本工具类指定http管理器
...
public class OAuthV1Client {
    ...
    public static OAuthV1 requestToken(OAuthV1 oAuth) throws Exception {
        ...
        String queryString = getOauthParams(OAuthConstants.OAUTH_V1_GET_REQUEST_TOKEN_URL, "GET",
                oAuth.getOauthConsumerSecret(), "",
                oAuth.getParamsList());
        Log.i(TAG, "requestToken queryString = " + queryString);

        String responseData = Q_HTTP_CLIENT.httpGet(OAuthConstants.OAUTH_V1_GET_REQUEST_TOKEN_URL,
                queryString);
     ...
     * 
     * @param responseData
     *            格式：oauth_token=OAUTH_TOKEN&vcode=VERIFY_CODE&openid=OPENID&
     *            openkey=OPENKEY
     * @param oAuth
     * @return
     * @throws Exception
     ...
     * @return
     */
    public static String getOauthParams(String url, String httpMethod, String consumerSecret,
            String tokenSecrect,
        ...
        String signature = generateSignature(aUrl, consumerSecret, tokenSecrect, httpMethod,
                queryParamsList);
     ...
     * @return
     */
    private static String generateSignature(URL url, String consumerSecret,
     ...
     * @return
     */
    private static String generateSignature(String base, String consumerSecret,
            String accessTokenSecret) {
        try {
            Mac mac = Mac.getInstance(hashAlgorithmName);
            String oAuthSignature = QStrOperate.paramEncode(consumerSecret)
                    + "&"
                    + ((accessTokenSecret == null) ? "" : QStrOperate
                            .paramEncode(accessTokenSecret));
     ...
     * @return
     */
    private static String generateSignatureBase(URL url, String httpMethod,
            List<NameValuePair> queryParamsList) {
