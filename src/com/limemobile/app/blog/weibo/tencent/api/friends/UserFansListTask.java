 ...
 */
public class UserFansListTask extends TAsyncTask<Friend> {
    public UserFansListTask(ILoadListener loadListener, IResultListener successListener) {
        this(null, loadListener, successListener);
    }

    public UserFansListTask(ViewGroup container, ILoadListener loadListener,
    ...
    public String callAPI(Object... params) {
        ...
        String fopneid = (String) params[3];
        String mode = Integer.toString((Integer) params[4]);
        String install = Integer.toString((Integer) params[5]);

        FriendsAPI friendsAPI = new FriendsAPI(TencentWeibo.getInstance().getOauthVersion());
        String response = null;
        try {
            response = friendsAPI.userFanslist(TencentWeibo.getInstance(), TencentWeibo.JSON, reqnum,
                    startindex, name, fopneid, mode, install);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
